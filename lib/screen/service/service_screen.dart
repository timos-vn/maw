...
import 'package:maw/screen/service/service_bloc.dart';
import 'package:maw/screen/service/service_state.dart';
import 'package:maw/screen/service/service_event.dart';
...
class _ServiceScreenState extends State<ServiceScreen>with SingleTickerProviderStateMixin{
  ...
  Widget buildPage(BuildContext context,ServiceState state){
      ...
      child: Scaffold(
        ...
        body: Container(
          ...
          child: Column(
            ...
            children: [
              appBar('Gói dịch vụ của tôi',context),
              Padding(
      ...
      child: Column(
        children: <Widget>[
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              CheckedContainer(paddingV: 15,content: 'Kho Enterbuy',checked: _bloc.checkingStore == 0,onTap: ()=> _bloc.add(ChangePageViewEvent(0)),checkedColor: subColor,),
              const SizedBox(width: Const.kDefaultPadding,),
              CheckedContainer(paddingV: 15,content: 'Kho Khách hàng',checked: _bloc.checkingStore == 1,onTap: ()=> _bloc.add(ChangePageViewEvent(1)),checkedColor: subColor,),
            ],
          ),
      ...
      children: [
        Padding(
          ...
          child: TabBar(
            ...
            tabs: List<Widget>.generate(_bloc.listTabService.length, (int index) {
              return Tab(
                text: _bloc.listTabService[index].toString(),
              );
            }),
          ),
        ),
        ...
        Expanded(
          child: TabBarView(
              controller: tabController,
              children: List<Widget>.generate(_bloc.listTabService.length, (int index) {
                for (int i = 0; i <= _bloc.listTabService.length; i++) {
                  if (i == index) {
